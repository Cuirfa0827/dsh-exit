...
- **🪟 关闭窗口自动退出（可选）**：`exitOnWindowClose: true` 时，所有窗口/标签页都关闭后约 5~6 秒服务自动退出；刷新页面不会误触发（页面刷新会先发 bye 信标、新页面在宽限期内重新注册，从而取消退出）
...
开启后：页面关闭时会通过 pagehide + sendBeacon 向宿主发送 bye 信标；宿主等 5 秒宽限（刷新场景下新页面会在这期间重新注册，从而取消退出），若已无任何页面则自动优雅退出。心跳仅作为崩溃兜底，TTL 放宽到 90 秒。多窗口同时打开时，关掉其中一个不会退出，关掉最后一个才退出。
...
- 窗口关闭检测 = pagehide + sendBeacon 的 bye 信标 + 5 秒宽限：刷新时新页面在宽限期内重新注册（hello）会取消退出；心跳（30 秒一次 + 90 秒 TTL）仅兜底崩溃/冻结页面，浏览器对后台标签页定时器节流不再导致误杀
...
> ℹ️ **自 0.3.0 起移除了附带的 macOS App 构建能力**（`macapp/`）以及命令行/launchd 启动器（`scripts/`）。本插件聚焦于「退出」职责；如需在退出后重新启动服务，请直接运行 `dsh web`。
