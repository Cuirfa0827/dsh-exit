...
const CSS = '' +
  ...
  '.dsx-btn-primary{background:var(--dsw-alias-state-error-primary);border-color:var(--dsw-alias-state-error-primary);color:var(--dsw-alias-state-error-label,#fff)}' +
  '.dsx-btn-primary:hover:not(:disabled){filter:brightness(1.08)}' +
  /* 退出成功后的整页反馈 */
...
function ExitButton({ wide }) {
  ...
  // 并压紧高度/间距（外壳容器默认高 50px、内容 42px 居中留白），让两个
  // 按钮紧凑相邻。
  react.useEffect(() => {
  ...
  async function doShutdown() {
    ...
    try {
      ...
      if (body && body.ok) {
        setPhase('done');
        // 通知宿主 App（macapp）关闭窗口；浏览器里该调用是无害的 no-op。
        notifyAppClosed();
        // 尽力关闭当前窗口（脚本打开的窗口 / PWA 应用窗口可关，普通标签页受浏览器限制）。
        window.setTimeout(() => {
  ...
  function notifyAppClosed() {
    try {
      window.webkit?.messageHandlers?.dshExit?.postMessage('done');
    } catch (e) { /* ignore */ }
  }
  ...
  if (phase === 'done') {
    return react.createElement('div', { className: 'dsx-done' },
      ...
      react.createElement('p', { className: 'dsx-done-desc' }, 'DSH 服务已优雅退出。此窗口现在可以安全关闭；需要再次使用时，启动 dsh web 或打开 DSH App 即可。'),
